...
public:
    ...
    void addValue(valueType value, bool (*comp)(valueType, valueType)){
        if (root == 0){
...
/// Can try equal = valueType::operator== :D
    void delValue(valueType value, bool (*comp)(valueType, valueType), bool (*equal)(valueType, valueType)){
        treeElement<valueType> *tmp = root;
        treeElement<valueType> *prev;
        //SEARCH
        while (tmp && !equal(value, tmp->getValue())){
            prev = tmp;
            if (comp(tmp->getValue(), value)){
                tmp = tmp->getRight();
            }else{
    ...
    void addValue(std::list<valueType> values, bool (*comp)(valueType, valueType)){
        while (!values.empty()){
            this->addValue(values.front(), comp);
            values.pop_front();
        }
    }

    void delValue(std::list<valueType> values, bool (*comp)(valueType, valueType), bool (*equal)(valueType, valueType)){
        while (!values.empty()){
            this->delValue(values.front(), comp, equal);
            values.pop_front();
        }
    }

    treeElement<valueType> *findValue(valueType value, bool (*comp)(valueType, valueType), bool (*equal)(valueType, valueType)){
        return findValue(new treeElement<valueType>(value), comp, equal);
    }

    treeElement<valueType> *findValue(treeElement<valueType> *value, bool (*comp)(valueType, valueType), bool (*equal)(valueType, valueType)){
        treeElement<valueType> *tmp = root;
        //SEARCH
        while (tmp && !equal(value->getValue(), tmp->getValue())){
            if (comp(tmp->getValue(), value->getValue())){
                tmp = tmp->getRight();
            }else{
                tmp = tmp->getLeft();
            }
